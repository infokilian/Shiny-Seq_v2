...
Module_Batch_effect_UI<-function(id)
{
  ...
  tagList(
    ...
    conditionalPanel(sprintf("input['%s']==2", ns("batch_choice")),
                      ...
                      actionButton(ns("helperr2"), "Help", icon = icon("question-circle"))),
    conditionalPanel(sprintf("input['%s']==3", ns("batch_choice")),
                     fluidRow(column (4,uiOutput(ns("text")))),
                     actionButton(ns("go"), "Go", icon = icon("paper-plane"))
    ...
    fluidRow(
      column(1,
             selectInput(ns("datachoice3")  ,label = h5("Select Data Type"), 
                         choices = list("Excel" = 1, "CSV" = 2),
                         selected = 1)),
      column(1, 
             br(),
             br(),
             downloadButton(ns('download_batch_corrected_Data'), 'Download batch corrected Data Table'))),
    fluidRow(
      column(10,    DT::dataTableOutput(ns("batch_corrected")),
                    gene_count_module_UI(ns("module1"))
             )),
    bsModal(ns("modalExample"), "Batch corrected PCA", ns("go"),size = "large",
            selectInput(ns("top_pca_sva"), label = h5("Should PCA be computed for:"), 
                        choices = list("Top 500 genes" = 1, "All" = 2),
                        selected = 2),
            fluidRow(textOutput(ns("plottitle1")),column(10,plotlyOutput(ns("plot1")))),
...
Module_Batch_effect<-function(input,output,session,conchoice,designchoice,dds.fc,normal)
{
  ...
  num_sv<-reactive({
    ...
    svaobj <-sva(rld, mod, mod0, n.sv=NULL)#n.sv)
  ...
  batch_design<-reactive({
    ...
    else if(as.numeric(input$batch_choice)==3)
    {
      ...
      if(n.sv>0){
        ...
        if(num_sv()[[1]] >6) svall<-sva(rld, mod, mod0, n.sv= 6)
        else svall<-sva(rld, mod, mod0, n.sv= num_sv()[[1]])
        ...
        var1<-substring(as.character(design(dds.fc()[[1]])),1,1)[1]
        var2<-substring(as.character(design(dds.fc()[[1]])),2)
        rownames(sv)<-pData[,1]
        temp<-''
        for (i in 1:n.sv){
        ...
        else
        {
          ...
          full_design<-paste(var1,temp,var2,sep = '')
    ...
    withProgress(message = 'Processing:',
                 detail = 'Removing batch effect .This may take a while...',value = 0, {
                   ...
                   if(as.numeric(input$batch_choice)==2)#
                     ...
                     {
                       batch<-colData(rld)[,as.numeric(input$batch[1])]
                       batch2<-colData(rld)[,as.numeric(input$batch[2])]
                     }
  ...
  output$download_batch_corrected_Data <- downloadHandler(
    filename = function() {
      if(as.numeric(input$datachoice3==1)){paste("Batch corrected Data Table.xlsx")}
      else if (as.numeric(input$datachoice3==2)){paste("Batch corrected Data Table.csv")}
    },
    
    content = function(file) {
      if(as.numeric(input$datachoice3==1)){
        wb <- createWorkbook()
        addWorksheet(wb, sheetName = "Batch corrected data")
        writeData(wb = wb, sheet = 1, x = batch_corrected()[[1]], colNames = T, rowNames = T)
        saveWorkbook(wb, file)
        }
      else if (as.numeric(input$datachoice3==2)){write.csv(batch_corrected()[[1]], file)}
    }
  )  
    ...
    local({
      ...
      output[[plotname]] <-renderPlotly({
        ...
        if(num_sv()[[1]]>6)
        {
          ...
          # Pause for 0.1 seconds to simulate a long computation.
          Sys.sleep(0.1)
          #Get which column you want to plot by
...
observeEvent(input$PCA,
             {
               if(input$batch_choice == 1){
                 normal<-list(normal(),colData(dds.fc()[[1]]),assay(dds.fc()[[2]]))
                 b$top <- callModule(PCA_module2,"pca_module_batch",data=normal, resize_factor=1)
               }

               else{ 
                 batch_pca<-batch_corrected()[[1]]
               batch<-list(assay(dds.fc()[[1]]),colData(dds.fc()[[1]]),batch_pca)
               b$top<-callModule(PCA_module3,"pca_module_batch",batch,resize_factor=1)
               }
             })  
