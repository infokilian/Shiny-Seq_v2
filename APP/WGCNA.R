...
WGCNA_module_UI<-function(id)
{
  ...
  tagList(
    ...
    fluidRow(
      column(1,
             selectInput(inputId= "datachoiceW"  ,label = h5("Select Data Type"), 
                         choices = list("Excel" = 1, "CSV" = 2),
                         selected = 1)),
      column(1, 
             br(),
             br(),
             downloadButton('download_wgcna_anova_Table', 'download ANOVA table for selected module'))),
...
observeEvent(input$go_wgcna_start,
             {
               ...
               network_cytoscape<- reactive({
                 ...
                 for(i in modules)
                 {
                   ...
                   cyt[length(cyt)+1] = exportNetworkToCytoscape(modTOM,
                                                                 edgeFile = paste("./wgcna files/CytoscapeInput-edges-", paste(i, collapse="-"), ".txt", sep=""),
                                                                 nodeFile = paste("./wgcna files/CytoscapeInput-nodes-", paste(i, collapse="-"), ".txt", sep=""),
                                                                 weighted = TRUE,
                                                                 threshold = threshold,
                                                                 nodeNames = modProbes,
                   ...
                   #print(cyt)
                   edgefile[length(edgefile)+1]<-paste("./wgcna files/CytoscapeInput-edges-", paste(i, collapse="-"), ".txt", sep="")
                   nodefile[length(nodefile)+1]<- paste("./wgcna files/CytoscapeInput-nodes-", paste(i, collapse="-"), ".txt", sep="")
                   file_name[length(file_name)+1]<-paste("./wgcna files/CytoscapeInput_GFC_", paste(i, collapse="-"), ".txt", sep="")
                   print("ok")
              ...
              output$download_wgcna_anova_Table <- downloadHandler(
                ...
                filename = function() 
                {
                  if(as.numeric(input$datachoiceW==1)){paste('ANOVA genes for ',modules$Var1[row],'.xlsx')}
                  else if (as.numeric(input$datachoiceW==2)){paste('ANOVA genes for ',modules$Var1[row],'.csv')}
                },
                  content = function(file) {
                    if(as.numeric(input$datachoiceW==1)){
                  #nam<-paste('ANOVA genes for ',modules$Var1[row])
                  M <- as.matrix(df)
                  wb <- createWorkbook()
                  addWorksheet(wb, sheetName = "ANOVA genes")
                  ...
                  #             col.names = TRUE, row.names = TRUE, append = FALSE)
                    ...
                    else if (as.numeric(input$datachoiceW==2)){write.csv(v$data[[1]], file)}
                  }
